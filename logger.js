...
setColor = (fg, bg) =>
  `${fg == undefined ? "" : fg}${bg == undefined ? "" : bg}%s${colors.reset}`;

assignFormat = (level) => {
  let tagFg =
    config[level].tag.fg == "default"
      ? ""
      : eval("colors." + config[level].tag.fg);
  let tagBg =
    config[level].tag.bg == "default"
      ? ""
      : eval("colors." + config[level].tag.bg);
  let msgFg =
    config[level].msg.fg == "default"
      ? ""
      : eval("colors." + config[level].msg.fg);
  let msgBg =
    config[level].msg.bg == "default"
      ? ""
      : eval("colors." + config[level].msg.bg);
